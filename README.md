...
Instead, **DSP** offers a number of powerful primitives and systematic design patterns for building architectures that orchasterate LMs and RMs. The **DSP** runtime deals with mapping the transformations you build into an effective implementation. For instance, **DSP** *annotates* few-shot demonstrations for the LM calls within your arbitrary pipeline automatically, and uses them to improve the quality of your transformations. Once you're happy with things, **DSP** can *compile* your program into a much cheaper version in which LM calls are transparently replaced with calls to a tiny LM created automatically by the **DSP** runtime.
