...
**DSP** programs take in-context learning to the next level, and they've been demonstrated to work well for knowledge-intensive tasks — i.e., those that require search, like answering questions or researching complex topics. Unlike vanilla in-context learning, you do not hard-code any few-shot prompts. We view "prompt engineering" much the same way as hyperparameter tuning in traditional ML: it's a final (and relatively minor) step that's best done _after_ building up an effective architecture and connecting its components together.
