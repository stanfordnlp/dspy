...
**DSPy is a framework for algorithmically optimizing the prompts and weights of LMs**, especially in programs that call LMs one or more times in a pipeline to solve a task. To use LMs to build a complex system without DSPy, you generally have to: (1) break the problem down into steps, (2) prompt your LM well until each step works well in isolation, (3) tweak the steps to work well together now, (4) generate synthetic data to tune each step, and (5) use that data to finetune smaller LMs to cut costs.
