...
class HotPotQATask(BaseTask):
    def __init__(self):
        ...
        self.evaluate_EM = Evaluate(devset=self.devset, metric=metric_EM, **kwargs)
        self.evaluate_retrieval = Evaluate(devset=self.devset, metric=gold_passages_retrieved, **kwargs)

        self.set_splits(TRAIN_NUM=100, DEV_NUM=100, EVAL_NUM=100)

    def get_program(self):
        return MultiHop(passages_per_hop=3) #TODO: make it so we can specify # of passages 
