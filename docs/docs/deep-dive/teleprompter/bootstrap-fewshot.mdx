...
When compiling a DSPy program, we generally invoke an optimizer, which takes the program, a training set, and a metric—and returns a new optimized program. This family of optimizers is focused on optimizing the few shot examples. Let's take an example of a Sample pipeline and see how we can use optimizers.
...
Now we have the baseline pipeline ready to use, so let's try using the `BootstrapFewShot` optimizer to make it even better!
...
2. Then to the teacher it add the demos by using `LabeledFewShot` optimizer.
