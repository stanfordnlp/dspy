...
We make use of the [GSM8K dataset](https://huggingface.co/datasets/gsm8k) and the OpenAI `gpt-4o-mini` model to simulate prompting tasks within DSPy.
...
# Set up the LM.
turbo = dspy.LM(model='openai/gpt-4o-mini', max_tokens=250)
dspy.configure(lm=turbo)

# Load math questions from the GSM8K dataset using `DataLoader`
dl = DataLoader()
...
gsm8k_trainset, gsm8k_devset = gsm8k['train'], random.sample(gsm8k['test'], 50)
```
