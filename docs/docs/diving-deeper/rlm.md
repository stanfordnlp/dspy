...
The base class for inputs that need custom loading. Implement `sandbox_setup`, `to_sandbox`, `sandbox_assignment`, and `rlm_preview`; override `sandbox_packages` when the interpreter must provision packages before setup. Package declarations must be a list of non-empty strings, and an interpreter used with them must implement `preload_packages(packages)`. `sandbox_setup` must import required dependencies so availability is checked during input injection. The class also defines a Pydantic schema hook, so a subclass can be a typed field in a signature, as in `data: DataFrame = dspy.InputField()`.

`dspy.Image` implements this contract and declares Pillow as its sandbox package. With the default `PythonInterpreter`, an image enters the sandbox as a string-compatible `DSPyImage`, and Pillow is preloaded and imported during image injection. Generated code can pass the image directly to `llm_query(..., images=[image])`, or convert embedded images with `image.to_pil()` and convert them back after manipulation with `DSPyImage.from_pil(...)`. As with other DSPy modules, wrap host-side PIL images explicitly with `dspy.Image(pil_image)` before passing them to RLM. Custom interpreters own their package environment but must implement the package-provisioning contract when handling package-bearing values.
