...
- Structured content is returned when the result carries it. When the tool's output schema declares the single-value envelope that SDK servers generate for typed returns (an object with exactly one required `result` property), the unpacker returns the inner value, so a tool annotated `-> int` returns an `int`. A tool that genuinely returns an object with one required `result` field has the same schema, so the unpacker checks the text content before unwrapping. SDK servers render a genuine object's text as the full JSON object, and in that case the object keeps its declared shape.
