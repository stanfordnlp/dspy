...
ReAct registers an extra `finish` tool — its arguments are the signature’s output fields, with JSON schemas derived from each field’s annotation and descriptions carrying the field’s `desc`, custom type description, and constraints — and the loop exits when the LM selects it, delivering the final outputs as the call’s arguments. An annotation whose JSON schema can’t be generated (a self-referencing Pydantic model, say) is advertised to the LM as a plain string instead, and the failure is logged at debug level; the value the LM sends back is still coerced to the declared annotation. Making termination a tool rather than a special-cased flag keeps the LM’s choice uniform: pick from the same list every turn, no extra parsing logic. A separate `max_iters` cap is the hard upper bound for cases where the LM never selects `finish`.
...
When the LM calls `finish`, ReAct coerces each argument to the corresponding output field’s annotation with the same `parse_value` the adapters use on the fallback path, and returns the coerced values directly, with the final `next_thought` as `reasoning` — no extra LM call. A signature that declares its own `reasoning` output field keeps control of it: the `finish` argument for that field wins over the thought. Sharing that coercion means the fast path is never stricter than the fallback: a bare `3` for a `str` output, an enum member’s name rather than its value, and a JSON-encoded string of the annotated type are all accepted here exactly as the extractor would have accepted them. An explicit `None` is the one exception — it is validated against the field’s precomputed `pydantic.TypeAdapter` instead, so it is a valid value for a nullable annotation such as `str | None` but falls back for a non-nullable one rather than being coerced into the string `"None"`. Only when an output field is absent from the args or fails validation — or the loop ends without a `finish` call — does ReAct hand the trajectory to a `dspy.ChainOfThought` over a fallback signature that includes the original output fields plus a `trajectory` input, whose job is to read the trajectory and produce the declared outputs in their correct types. The fast path saves one LM round-trip per call; the fallback keeps the module robust when the final arguments don’t validate. Either way, only the declared output fields are read — an argument the LM invents beyond them is recorded in the trajectory and otherwise ignored — and the module’s `trajectory` dict is applied last, so a signature that declares a `trajectory` output field of its own gets the dict back rather than the value it produced for that field.

Two notes follow from `finish` being handled before tool dispatch. First, the `finish` tool’s function runs outside `Tool.__call__`’s strict argument validation (its args are instead coerced by the fast path), but its `on_tool_start`/`on_tool_end` callbacks (e.g. tracing integrations) still fire on every `finish` call, fast path or fallback. Second, a `dspy.streaming.StreamListener` bound to a final output field (e.g. `answer`) auto-binds to the fallback extractor, the only predictor whose signature contains that field; on runs where the fast path returns directly, the extractor never runs and the listener emits nothing — the final value appears only inside the react predictor’s `next_tool_args` JSON.
