...
- Structured content is returned when the result carries it. When the tool's output schema declares the single-value envelope that SDK servers generate for typed returns (an object with exactly one required `result` property), the unpacker returns the inner value, so a tool annotated `-> int` returns an `int`. A tool that genuinely returns an object with one required `result` field has the same schema, so the unpacker only unwraps when the text content matches the inner value, which is how SDK servers render a wrapped return. A result whose text renders the full object, or that carries no text content at all, keeps its declared shape.
