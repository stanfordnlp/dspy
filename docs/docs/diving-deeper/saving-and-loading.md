...
**`Adapter.dump_state()`**  
Saves the adapter’s concrete class and constructor configuration. `adapter: null` preserves inheritance from `dspy.settings`, and legacy state without an adapter field remains compatible. `TwoStepAdapter` also saves its extraction LM using the usual LM-state sanitization. Custom subclasses can extend `dump_state` and `load_state` for additional constructor state. Full-program saves instead cloudpickle the complete adapter object, including arbitrary attributes.
