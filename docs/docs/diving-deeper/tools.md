...
The bridge supports both the camelCase result fields in MCP SDK v1 and their snake_case replacements in v2. It returns structured MCP results by default when available, falling back to the existing conversion when they are absent. Pass `result_mode="text"` to preserve the legacy text/non-text behavior.
...
**`Tool.from_mcp_tool(session, tool, *, result_mode="structured")`** → `Tool`
Wraps a remote MCP tool as an asynchronous DSPy tool. Set `result_mode="text"` to use the legacy text/non-text conversion.
