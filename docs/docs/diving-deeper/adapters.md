...
**`dspy.XMLAdapter(callbacks=None, use_json_adapter_fallback=True)`**  
`<field_name>value</field_name>` tags, with output values escaped on the wire: the formatter writes `&` as `&amp;` and `<` as `&lt;`, the prompt instructs the model to do the same, and `parse` decodes the entities — so the format → parse round trip is lossless, even for a value containing its own closing tag. Escaping `&` as well is what makes the scheme reversible: a value that already spells `&lt;` or `&amp;` comes back byte-identical rather than corrupted by the decode. The parser is a single linear tag scan, robust to whitespace.

A completion that ignores the instruction and leaves `<` raw falls back to the tag machinery. A value that is itself a same-named element (`<code><code>x</code></code>`) is recovered in full, but a value that merely *contains* its own closing tag is indistinguishable from one followed by trailing commentary — `parse` raises `AdapterParseError` when the surplus closing tag is left over in open text with real text before it, and also when a nested value's span holds another output field, where widening would swallow that field and stopping early would silently drop the rest of the value. Two shapes escape those checks and are still truncated silently: a value that *ends* with its own closing tag, where only whitespace separates the two readings; and a value whose nested reading was rejected for borrowing structure from another element, where a later `<tag>...</tag>` block — not necessarily one of your fields — either covers the surplus tag or is all that stands between it and the value. Note the decode applies to every completion, compliant or not, so a legacy response that spells `&lt;` or `&amp;` literally now reads back decoded. XMLAdapter extends ChatAdapter and so inherits `use_json_adapter_fallback`, which under `__call__` retries an `AdapterParseError` response through JSONAdapter, costing a second LM call — pass `use_json_adapter_fallback=False` if you’d rather see the error. Under `dspy.streamify`, streamed chunks are entity-decoded the same way, and a raw value of the nested shape is buffered and delivered as one chunk rather than token by token, because the tag that ends it isn’t known until it arrives, so the streamed value matches what `parse` returns for it. For the ambiguous sibling shape above the stream can only emit the lazy reading as tokens arrive — a sent chunk can’t be recalled — and the run then ends in the `parse` error rather than a prediction. Streaming still starts a field at its opening tag as the tokens arrive, so a raw value that mentions a later field’s tag before that field opens can start the wrong one — the same reading `parse` would give the tokens seen so far. See [Streaming](../tutorials/streaming/index.md).
