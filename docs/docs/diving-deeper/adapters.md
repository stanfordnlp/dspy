...
`<field_name>value</field_name>` tags. The parser is a single linear tag scan, robust to whitespace. A value that is itself a same-named element (`<code><code>x</code></code>`) is recovered in full, but the format does not escape values, so a value that merely contains its own closing tag is indistinguishable from one followed by trailing commentary — `parse` raises `AdapterParseError` when the surplus closing tag is left over in open text with real text before it. Two shapes escape that check and are still truncated silently: a value that *ends* with its closing tag, and a value whose surplus closing tag is covered by, or separated from the value by, any later balanced block — that block does not have to belong to a declared field, so an unrelated `<note>…</note>` conceals it too. Use ChatAdapter or JSONAdapter for content that can contain the closing tag; note that under `__call__`, ChatAdapter’s `use_json_adapter_fallback` will retry such a response through JSONAdapter, costing a second LM call.
