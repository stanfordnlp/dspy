...
`<field_name>value</field_name>` tags. The parser is a single linear tag scan, robust to whitespace. A value that is itself a same-named element (`<code><code>x</code></code>`) is depth-balanced, so it has exactly one reading and is recovered in full. Beyond that the format doesn’t escape values, so a value containing its own closing tag is indistinguishable from a value followed by trailing commentary — `<answer>a</answer> b</answer>` may mean either — and `parse` reports an `AdapterParseError` rather than silently returning one of the two readings. Two cases stay quiet: a surplus closing tag separated from the value by nothing but whitespace or whole balanced blocks, which reads as the model closing the field twice; and a value that merely *ends* with its closing tag (`<code>before </code></code>`), where the scan closes the block early and leaves nothing surplus to detect — that shape still truncates, as it did on earlier versions. Use ChatAdapter or JSONAdapter for content that can contain the closing tag; note that under `__call__`, ChatAdapter’s `use_json_adapter_fallback` will retry such a response through JSONAdapter, costing a second LM call.
