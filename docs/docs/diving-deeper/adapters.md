...
`<field_name>value</field_name>` tags. The parser is a regex (`r"<(\w+)>(.*?)</\1>"` with `DOTALL`); it’s robust to whitespace but doesn’t tolerate nested tags of the same name. Because the format doesn’t escape values, a value containing its own closing tag is indistinguishable from a value followed by trailing commentary — where `parse` can see that ambiguity it reports an `AdapterParseError` instead of silently returning the truncated value. Two cases stay quiet: a surplus closing tag separated from the value by nothing but whitespace or whole balanced blocks, which reads as the model closing the field twice; and a value that re-opens the field’s own tag (`<code><code>x</code></code>`) or ends with its closing tag (`<code>before </code></code>`), where the regex closes the block early and the truncation is still silent — the same limitation as on earlier versions. Use ChatAdapter or JSONAdapter for content that can contain the closing tag; note that under `__call__`, ChatAdapter’s `use_json_adapter_fallback` will retry such a response through JSONAdapter, costing a second LM call.
