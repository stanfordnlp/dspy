...
`<field_name>value</field_name>` tags. The parser is a single linear tag scan, robust to whitespace, and a value that is itself a same-named element (`<code><code>x</code></code>`) is recovered in full. The format has no escaping, though, so a value that merely *contains* its own closing tag is indistinguishable from one followed by trailing commentary. `parse` raises `AdapterParseError` when the surplus closing tag is left over in open text with real text before it. Two shapes escape that check and are still truncated silently: a value that *ends* with its own closing tag, where only whitespace separates the two readings; and a value whose nested reading was rejected, where a later `<tag>...</tag>` block — not necessarily one of your fields — either covers the surplus tag or is all that stands between it and the value. Use ChatAdapter or JSONAdapter for content that can contain the closing tag; note that XMLAdapter extends ChatAdapter and so inherits `use_json_adapter_fallback`, which under `__call__` retries such a response through JSONAdapter, costing a second LM call — pass `use_json_adapter_fallback=False` if you’d rather see the `AdapterParseError`. Under `dspy.streamify`, a value of the nested shape is buffered and delivered as one chunk rather than token by token, because the tag that ends it isn’t known until it arrives, so the streamed value matches what `parse` returns for it. Streaming still starts a field at its opening tag as the tokens arrive, so a value that mentions a later field’s tag before that field opens can start the wrong one — the same reading `parse` would give the tokens seen so far. See [Streaming](../tutorials/streaming/index.md).
