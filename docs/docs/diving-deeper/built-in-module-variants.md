...
Same shape as `BestOfN`, with feedback generation between attempts. At initialization, `Refine` adds a `hint_` input field with a context-aware default to each sub-predictor’s signature. After a failed attempt, it builds a snapshot — the module’s source code, per-predictor signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)`. That call returns advice keyed by module name. On the next attempt, `Refine` scopes that advice to the module call, so normal predictor default handling supplies each hint without replacing the active adapter or modifying signatures at runtime.
