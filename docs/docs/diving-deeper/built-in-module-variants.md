...
Same shape as `BestOfN`, with feedback generation between attempts. After a failed attempt, `Refine` builds a snapshot — the module’s source code, per-predictor signatures, per-predictor I/O from the trace, the reward function’s source — and feeds it to an internal `dspy.Predict(OfferFeedback)`. That call returns advice keyed by module name. On the next attempt, `Refine` wraps each sub-predictor’s resolved adapter so its signature gains a `hint_` input field carrying its slice of the advice. The wrapper is installed as an instance adapter on the per-attempt deep copy — preserving each predictor’s own resolution (instance adapter, then settings, then `ChatAdapter`) — so it exists only for that attempt and the hints don’t leak into the final trace.
