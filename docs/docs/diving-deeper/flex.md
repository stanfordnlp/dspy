...
Inputs cross the same boundary in the other direction. A value typed as a pydantic model or dataclass reaches the generated `forward` as a dict-like record that supports both `place.name` and `place["name"]`, and so does each field of a bridged predictor's result; the proposer is told the model's field names alongside the type. When the generated code hands such a value back to a bridged predictor whose signature declares the model, the host rebuilds the model before the call, so the predictor sees the same `Place` it would outside `Flex`. `dspy.Type` values (`dspy.Image`, `dspy.Audio`, …) are different: they cross as opaque strings and the original object is restored on the host.
