...
def version_tuple(version: str) -> tuple[int, int, int, int, int]:
    ...
    if not match or int(match.group(1)) < 3:
        raise argparse.ArgumentTypeError("expected an X.Y.Z release version at or after 3.0.0")
    major, minor, patch, prerelease, prerelease_number = match.groups()
    phase = {"a": 0, "b": 1, "rc": 2, None: 3}[prerelease]
    return int(major), int(minor), int(patch), phase, int(prerelease_number or 0)
...
def legacy_redirects(site: Path, identifier: str) -> dict[str, str]:
    ...
    for page in sorted(site.rglob("*.html")):
        ...
        if top_level == "current" or RELEASE_VERSION.fullmatch(top_level) or re.fullmatch(r"\d+\.\d+", top_level):
            continue
    ...
    *,
    ...
    with working_directory(repository):
        with commands.deploy(
            config,
            identifier,
            "Current" if current else identifier,
            aliases,
            update_aliases=True,
        ...
        if current and (root is None or f"url={identifier}/" not in root):
