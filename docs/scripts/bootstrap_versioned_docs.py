...
def bootstrap_release(
    *,
    ...
    try:
        ...
        package_source = "tag-built-wheel" if version == "3.1.1" else "pypi-wheel"
        if package_source == "pypi-wheel":
...
def main() -> None:
    ...
    releases = [version for version in manifest["releases"] if not args.only or version in args.only]
    unknown = set(args.only) - set(releases)
    if unknown:
