...
class Retriever(ABC):
    def __init__(self, embedder: Optional[Embedder] = None, k: int = 5, callbacks: Optional[List[Any]] = None, cache: bool = False):
        """
        Interface for composing retrievers in DSPy to return relevant passages or documents based on a query.

        Args:
            embedder (Optional[Embedder]): An instance of `dspy.Embedder` used to compute embeddings
