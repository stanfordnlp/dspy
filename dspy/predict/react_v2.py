...
# Input field names ReActV2 reserves for its internal agent loop.
# A user signature declaring these as input fields would be clobbered by the agent's
# conversation history and tool-list dispatch in `_make_react_signature`.
...
class ReActV2(Module):
    ...
    def __init__(self, signature: type[Signature], tools: list[Callable | Tool], max_iters: int = 20):
        ...
        if reserved_inputs:
            ...
            raise ValueError(
                f"Input field name(s) {names} are reserved by ReActV2 for internal agent execution. "
                "Rename these input fields on your signature."
            )
