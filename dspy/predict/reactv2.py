...
def _build_submit_tool(signature: type["Signature"]) -> Tool:
    outputs = ", ".join([f"`{key}`" for key in signature.output_fields])
    output_args = {}
    output_arg_types = {}
    for name, field in signature.output_fields.items():
        annotation = getattr(field, "annotation", str)
        output_args[name] = _json_schema_for_annotation(annotation)
...
def _inline_json_schema_refs(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    def resolve(value: Any) -> Any:
        ...
        return {key: resolve(item) for key, item in value.items() if key not in {"$defs", "definitions"}}
...
class ReActV2(Module):
    def __init__(self, signature: type["Signature"] | str, tools: list[Callable], max_iters: int = 20):
        ...
        optional_input_fields = deepcopy(signature.input_fields)
        for field in optional_input_fields.values():
            field.default = None

        react_signature = (
            dspy.Signature(optional_input_fields, self._build_instructions())
            .append("history", dspy.InputField(), type_=dspy.History)
            .append("tools", dspy.InputField(), type_=list[dspy.Tool])
            .append("tool_call_results", dspy.InputField(default=None), type_=dspy.ToolCallResults)
    ...
    def _build_instructions(self) -> str:
        inputs = ", ".join([f"`{key}`" for key in self.signature.input_fields])
        outputs = ", ".join([f"`{key}`" for key in self.signature.output_fields])
    ...
    def forward(self, **input_args):
        history_arg = input_args.pop("history", None)
        if history_arg is None:
            history = History()
        elif isinstance(history_arg, History):
            history = history_arg
        else:
            history = History.model_validate(history_arg)
    ...
    def _forced_submit(self, history: History, input_args: dict[str, object], break_reason: str | None = None):
        ...
        call_config = adapter.force_tool_call_config("submit") if hasattr(adapter, "force_tool_call_config") else {}
        ...
        try:
            ...
            result = {key: getattr(extract, key) for key in self.signature.output_fields if hasattr(extract, key)}
