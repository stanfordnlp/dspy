...
class BridgeRuntime:
    ...
    def forward(self, inputs: dict[str, Any]) -> Any:
        ...
        try:
            tools = {CONSTRUCT_TOOL: invocation.construct, CALL_TOOL: invocation.call}
            tools.update({name: _restoring_entrypoint(fn, originals) for name, fn in self._tool_callables().items()})
            _bind_interpreter(interp, {**interp.tools, **tools})
