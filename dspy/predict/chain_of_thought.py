...
class ChainOfThought(Predict):
    def __init__(self, signature, rationale_type=None, activated=True, **config):
        ...
        *keys, last_key = signature.kwargs.keys()
        ...
        DEFAULT_RATIONALE_TYPE = internals.Type(prefix="Reasoning: Let's think step by step in order to",
        ...
        self.extended_signature = internals.Template(signature.instructions, **extended_kwargs)
    
    def forward(self, **kwargs):
        signature = self.signature

        if self.activated is True or (self.activated is None and isinstance(internals.settings.lm, internals.GPT3)):
