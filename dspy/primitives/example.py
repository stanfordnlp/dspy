...
class Example:
    ...
    def dump_state(self):
        state_keys = ["_store", "_demos"]
        return {k: getattr(self, k) for k in state_keys} | {
            "_input_keys": list(self._input_keys) if self._input_keys else None
        }

    def load_state(self, state):
        _input_keys = state.pop("_input_keys") if "_input_keys" in state else None
        self._input_keys = set(_input_keys) if _input_keys else None
