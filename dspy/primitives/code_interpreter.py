...
class CodeInterpreter(Protocol):
    ...
    Optional input preparation:
        RLM calls ``prepare_inputs(inputs)`` if supplied, before building variable
        descriptions or reconstructing SandboxSerializable values. It must return
        a new input dictionary without mutating caller-owned values. For example,
        Monty represents images as URL/data-URI strings instead of Pyodide objects.
        This method is optional and is not part of the required protocol.
