...
class SignatureOptimizer(Teleprompter):
    ...
    def _check_candidates_equal(self, candidate1, candidate2):
        for p1, p2 in zip(
            ...
            if (
                not p1.extended_signature.instructions
                == p2.extended_signature.instructions
            ):
                return False
        ...
        for predictor in module.predictors():
            ...
            *_, last_key = predictor.extended_signature.fields.keys()
            if hasattr(predictor, "extended_signature"):
                ...
                basic_prefix = predictor.extended_signature.fields[
                    last_key
                ].json_schema_extra["prefix"]
            else:
                basic_instruction = predictor.extended_signature1.instructions
                basic_prefix = predictor.extended_signature1.fields[
                    last_key
                ].json_schema_extra["prefix"]
            ...
            instruct.completions.add_example(
                Example(
                ...
                for c_i, c in enumerate(candidates_):
                    ...
                    if hasattr(p_new, "extended_signature"):
                        ...
                        *_, last_key = p_new.extended_signature1.fields.keys()
                        p_new.extended_signature1 = (
                            p_new.extended_signature1.with_instructions(
                                instruction
                            ).with_updated_fields(last_key, prefix=prefix)
                        )
                        *_, last_key = p_new.extended_signature2.fields.keys()
                        p_new.extended_signature2 = (
                            p_new.extended_signature2.with_instructions(
                                instruction
                            ).with_updated_fields(last_key, prefix=prefix)
                        )
                    ...
                    if self.verbose:
                        print(
                            f"At Depth {d}/{self.depth}, Evaluating Prompt Candidate #{c_i}/{len(candidates_)} for Predictor {p_i} of {len(module.predictors())}."
                        )
                    ...
                    *_, last_key1 = p_old.extended_signature1.fields.keys()
                    p_new.extended_signature1 = (
                        p_new.extended_signature.with_instructions(
                            ...
                            last_key1, prefix=best_candidate["prefix"]
                        )
                    )
                    *_, last_key2 = p_old.extended_signature2.fields.keys()
                    p_new.extended_signature2 = (
                        p_new.extended_signature.with_instructions(
                            ...
                            last_key2, prefix=best_candidate["prefix"]
                        )
                    )
                if self.verbose:
                    print(
