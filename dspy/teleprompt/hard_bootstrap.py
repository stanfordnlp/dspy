    ...
    def _bootstrap_hard_examples(self, *, max_bootsraps=None):
        ...
        for round_idx in range(self.max_rounds):
            for example_idx, example in enumerate(tqdm.tqdm(self.trainset)):
                ...
                if (total_attempts-total_successes) / total_attempts > min_failed_attempts/total_attempts and total_successes > 0:
                    if predictor_name not in self.name2traces:
                        self.name2traces[predictor_name] = []
    ...
    def _bootstrap_one_example(self, example, round_idx=0):
        ...
        try:
            with dsp.settings.context(trace=[], **self.teacher_settings):
                ...
                lm = dsp.settings.lm
                # breakpoint()
                # lm = lm.copy(temperature=self.temperature + 0.001 * round_idx) if round_idx > 0 else lm
                original_lm_temp = lm.kwargs["temperature"]
                if round_idx > 0:
                    lm.kwargs["temperature"] = self.temperature + 0.1 * round_idx
                new_settings = dict(lm=lm) if round_idx > 0 else {}
                # print(f'bootstrapping with temperature {lm.kwargs["temperature"]}')
                ...
                lm.kwargs["temperature"] = original_lm_temp
        ...
        if success:
            for step in trace:
                ...
                # if add_example_to_traces:
                #     name2traces[predictor_name].append(demo)
        
        return success, predictor_name, demo
