...
class MIPROvmy(Teleprompter):
    ...
    def compile(
        ...
        *,
        ...
        if self.prompt_pool == {} and prompt_pool is not None:
        ...
        # Set random seeds
        seed = seed_opt or self.seed_opt
        self._set_random_seeds(seed_opt)
    ...
    def _optimize_prompt_parameters(
        ...
        def objective(trial):
            ...
            trial_num = trial.number # + 1 # not logging defualt run
            if minibatch:
            ...
            self.recorder.record(rollout=total_eval_calls, validation_error=1-score/100, instruction_idx=0, demos_idx=0)
    ...
    def _select_and_insert_instructions_and_demos(
        ...
        chosen_params = []
        raw_chosen_params = {}
        chosen_params_tuple_int = []

        for i, predictor in enumerate(candidate_program.named_predictors()):
            cur_module_name, cur_module = predictor[0], predictor[1]
            demos_idx = 0
            resample = True
            while resample:
                # trial.study.sampler.reseed_rng()
                # Select instruction
                instruction_idx = trial.suggest_categorical(
                    f"{i}_predictor_instruction", range(len(instruction_candidates[cur_module_name]))
                )
                # Select demos if available
                if demo_candidates:
                    demos_idx = trial.suggest_categorical(f"{i}_predictor_demos", range(len(demo_candidates[cur_module_name])))
                resample = False
                
                selected_instruction = instruction_candidates[cur_module_name][instruction_idx]
                updated_signature = get_signature(cur_module).with_instructions(selected_instruction)
                set_signature(cur_module, updated_signature)
                ...
                # Select demos if available
                if demo_candidates:
                    cur_module.demos = demo_candidates[cur_module_name][demos_idx]
                ...
                chosen_params_tuple_int.append((instruction_idx, demos_idx))     
    ...
    def _get_param_distributions(self, program, instruction_candidates, demo_candidates):
        ...
        for i, predictor in enumerate(program.named_predictors()):
            cur_module_name, cur_module = predictor[0], predictor[1]
            param_distributions[f"{i}_predictor_instruction"] = CategoricalDistribution(
                range(len(instruction_candidates[cur_module_name]))
            )
            if demo_candidates:
                param_distributions[f"{i}_predictor_demos"] = CategoricalDistribution(range(len(demo_candidates[cur_module_name])))
