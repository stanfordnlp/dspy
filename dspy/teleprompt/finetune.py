...
class PEZFinetune(Teleprompter):
    def __init__(self, metric=None, teacher_settings=None, multitask=True):
        """
        Fine-tune the model using optimized prompts generated by PEZ.

        Parameters
        ----------
        metric: Callable
            Metric function to evaluate the fine-tuning process.
        teacher_settings: dict, optional
            Settings for the teacher model.
        multitask: bool, default True
    ...
    def compile(
        ...
        """
        Fine-tune the student model on top of PEZ-optimized prompts.
        """
        if teacher is None:
            print("WARNING: Using vanilla teacher for fine-tuning.")

        # Compile and get optimized prompts via PEZFewshot
        compiled = self.teleprompter.compile(student, teacher=teacher, trainset=trainset)
        finetune_data = self._prepare_finetune_data(compiled, trainset)

        # Fine-tune the model using the finetune_data
        finetuned_model = self._finetune_model(finetune_data, target, bsize, accumsteps, lr, epochs, bf16, int8,
                                               path_prefix)

        # Return fine-tuned student with optimized prompts
        return finetuned_model

    def _prepare_finetune_data(self, compiled, trainset):
        # Prepare the fine-tuning data based on compiled student/teacher and training set
        finetune_data = {"prompt": [], "completion": []}  # Collect prompt and completion pairs
        # Collect relevant data from student/teacher here
        return finetune_data

    def _finetune_model(self, finetune_data, target, bsize, accumsteps, lr, epochs, bf16, int8, path_prefix):
        # Actual fine-tuning logic using finetune_data and the provided model parameters
        print(f"Fine-tuning {target} model on the finetune_data...")
        return student  # Return fine-tuned student model
