...
class RandomSelection(Teleprompter):
    def __init__(
        self,
        metric: Callable,
    ...
    def _optimize_prompt_parameters(
        ...
        def objective(trial):
            ...
            trial_num = trial.number # + 1 # not logging dedualt run
            if minibatch:
    ...
    def _select_and_insert_instructions_and_demos(
        ...
        for i, predictor in enumerate(candidate_program.predictors()):
            ...
            max_resamples = 10
            demos_idx = 0
            while resample_num < max_resamples:
                # Select instruction Randomly
                instruction_idx = self.rng.choice(range(len(instruction_candidates[i])))
                # instruction_idx = trial.suggest_categorical(
                #     f"{i}_predictor_instruction", range(len(instruction_candidates[i]))
                # )
                if demo_candidates:
                    # Select demos Randomly
                    demos_idx = self.rng.choice(range(len(demo_candidates[i])))
                    # demos_idx = trial.suggest_categorical(f"{i}_predictor_demos", range(len(demo_candidates[i])))

                # Check if sampled program has never ran before
                resample = False
                for t in range(1, trial_num):
                    if trial_logs[t][f"{i}_predictor_instruction"] == instruction_idx:
                        if demo_candidates and trial_logs[t][f"{i}_predictor_demos"] == demos_idx: # if we already tried this instruction X demos before
                            logger.info(f"Sampled program has already been evaluated in trial {t}, re-sampling...")
                            resample = True
                resample_num = resample_num + 1 if resample else max_resamples

                if resample:
                    resample_num += 1
                else:
                    resample_num = max_resamples
