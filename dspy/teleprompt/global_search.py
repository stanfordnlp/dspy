...
class GlobalSearch(Teleprompter):
    def __init__(
        self,
        metric: Callable,
        split: int,
    ...
    def _optimize_prompt_parameters(
        ...
        default_score, _ = eval_candidate_program(
            len(valset[:100]), valset[:100], program, evaluate, self.rng, return_all_scores=True
        )
        ...
        for trial_num in range(num_trials): # num_trials = 250 / num_split
            prompt_idx = num_trials * (split - 1) + trial_num
            instruction_idx = prompt_idx // 50
            demos_idx = prompt_idx % 50

            if minibatch:
                logger.info(f"== Trial {trial_num+1} / {adjusted_num_trials} - Minibatch ==")
            else:
    ...
    def _log_minibatch_eval(
        ...
        logger.info(
            f"{'=' * len(f'== Trial {trial_num + 1} / {adjusted_num_trials} - Minibatch Evaluation ==')}\n\n"
        )
    ...
    def _log_normal_eval(
        ...
        logger.info(f"{'=' * len(f'===== Trial {trial_num + 1} / {num_trials} =====')}\n\n")
