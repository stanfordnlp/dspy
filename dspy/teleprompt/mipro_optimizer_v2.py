...
compiled_prompt_opt = teleprompter.compile(program, trainset=trainset, self.max_bootstrapped_demos=3, self.max_labeled_demos=5)
...
compiled_prompt_opt = teleprompter.compile(program, trainset=trainset[:TRAIN_NUM], num_trials=100, self.max_bootstrapped_demos=3, self.max_labeled_demos=5)
...
class MIPROv2(Teleprompter):
    ...
    def _set_hyperparams_from_run_mode(
        ...
        num_vars = (
            len(program.predictors()) if zeroshot_opt else len(program.predictors()) * 2
        )  # (multiplying by two to account for few-shot examples + instruction variables)

        num_trials = auto_run_settings[self.auto]["num_trials"]
        valset = create_minibatch(
            valset, batch_size=auto_run_settings[self.auto]["val_size"]
        )
        minibatch = len(valset) > MIN_MINIBATCH_SIZE
        self.num_candidates = int(
            np.round(np.min([num_trials * num_vars, (3 / 2 * num_trials) / num_vars]))
        )
    ...
    def compile(
        ...
        *,
        ...
        if max_bootstrapped_demos or max_labeled_demos:
            self.max_bootstrapped_demos = max_bootstrapped_demos
            self.max_labeled_demos = max_labeled_demos
        ...
        if not valset:
            if len(trainset) < 2:
                raise ValueError(
                    "Trainset must have at least 2 examples if no valset specified, or at least 1 example with external validation set."
                )

            valset_size = min(
                500, max(1, int(len(trainset) * 0.80))
            )  # 80% of trainset, capped at 500
            ...
            if len(valset) < 1:
                raise ValueError(
                    "Validation set must have at least 1 example if specified."
                )
        ...
        if minibatch and minibatch_size > len(valset):
            raise ValueError(
                f"Minibatch size cannot exceed the size of the valset.  Note that your validation set contains {len(valset)} examples.  Your train set contains {len(trainset)} examples."
            )
        ...
        if not minibatch:
            ...
            estimated_task_model_calls_wo_module_calls = minibatch_size * num_trials + (
                len(trainset) * (num_trials // minibatch_full_eval_steps)
            )  # B * T * P
        ...
        if run:
            ...
            if self.max_bootstrapped_demos == 0 and self.max_labeled_demos == 0:
                self.max_bootstrapped_demos_for_candidate_gen = (
                    BOOTSTRAPPED_FEWSHOT_EXAMPLES_IN_CONTEXT
                )
                self.max_labeled_demos_for_candidate_gen = (
                    LABELED_FEWSHOT_EXAMPLES_IN_CONTEXT
                )
            else:
                self.max_bootstrapped_demos_for_candidate_gen = self.max_bootstrapped_demos
                self.max_labeled_demos_for_candidate_gen = self.max_labeled_demos
            ...
            try:
                demo_candidates = create_n_fewshot_demo_sets(
                    ...
                    trainset=trainset,
                    max_labeled_demos=self.max_labeled_demos_for_candidate_gen,
                    max_bootstrapped_demos=self.max_bootstrapped_demos_for_candidate_gen,
                    metric=self.metric,
                    max_errors=self.max_errors,
            ...
            default_score = eval_candidate_program(
                len(valset), valset, program, evaluate
            )
            ...
            def create_objective(
                ...
                def objective(trial):
                    nonlocal best_program, best_score, best_scoring_trial, trial_logs, total_eval_calls, scores, full_eval_scores  # Allow access to the outer variables
                    ...
                    for i, p_new in enumerate(candidate_program.predictors()):
                        ...
                        chosen_params.append(
                            f"Predictor {i+1}: Instruction {instruction_idx}"
                        )
                        if demo_candidates:
                            ...
                            chosen_params.append(
                                f"Predictor {i+1}: Few-Shot Set {demos_idx}"
                            )
                    ...
                    trial_logs[trial.number + 1]["program_path"] = (
                        save_candidate_program(
                            candidate_program,
                            self.log_dir,
                            trial.number + 1,
                    ...
                    trial_logs[trial.number + 1]["full_eval"] = batch_size >= len(
                        valset
                    )
                    ...
                    if minibatch:
                        ...
                        print(
                            f"Minibatch Score Trajectory: {', '.join([f'{s}' for s in scores])}"
                        )
                        trajectory = ", ".join([f"{s}" for s in full_eval_scores])
                        print(f"Full Eval Trajectory: {trajectory}")
                        print(f'{"="*len(f"== Minibatch Trial {trial.number+1} / {num_trials} ==")}\n\n')
                    else:
                        print(f"Score: {score} with parameters {chosen_params}.")
                        if best_score_updated:
                            print(
                                f"{GREEN}New best score updated!{ENDC} Score: {best_score} on trial {best_scoring_trial}."
                            )
                        print()
                        print(
                            f"Score Trajectory: {', '.join([f'{s}' for s in scores])}"
                        )
                        print(
                            f"Best score so far: {best_score} on trial {best_scoring_trial}."
                        )
                        print(f'{"="*len(f"===== Trial {trial.number+1} / {num_trials} =====")}\n\n')

                    # If we're doing minibatching, check to see if it's time to do a full eval
                    if minibatch and (
                        ((trial.number + 1) % minibatch_full_eval_steps == 0)
                        or (trial.number + 1 == num_trials)
                    ):
                        print(
                            f"===== Full Eval {len(fully_evaled_param_combos)+1} ====="
                        )

                        # Save old information as the minibatch version
                        trial_logs[trial.number + 1]["mb_score"] = score
                        trial_logs[trial.number + 1]["mb_program_path"] = trial_logs[
                            trial.number + 1
                        ]["program_path"]

                        # Identify our best program (based on mean of scores so far, and do a full eval on it)
                        highest_mean_program, mean, combo_key = (
                            get_program_with_highest_avg_score(
                        ...
                        if trial.number + 1 // minibatch_full_eval_steps > 0:
                            print(
                                f"Doing full eval on next top averaging program (Avg Score: {mean}) so far from mini-batch trials..."
                            )
                        else:
                            print(
                                f"Doing full eval on top averaging program (Avg Score: {mean}) so far from mini-batch trials..."
                            )
                        ...
                        if full_val_score > best_score:
                            print(
                                f"{GREEN}Best full eval score so far!{ENDC} Score: {full_val_score}"
                            )
