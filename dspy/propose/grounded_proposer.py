        ...
        if self.use_task_demos:
            ...
            for example in demo_candidates[pred_i][demo_set_i]:
                if "augmented" in example.keys():
                    ...
                    task_demos.append(example_string)
                    curr_demos_num += 1
...
class GroundedProposer(Proposer):
    ...
    def propose_instructions_for_program(
        ...
        if self.set_history_randomly:
            ...
            if self.verbose:
                logging.info(f"Use history T/F: {self.use_instruct_history}")
        ...
        if not demo_candidates:
            if self.verbose:
                logging.info("No demo candidates provided. Running without task demos.")
