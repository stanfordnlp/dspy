...
from dspy.teleprompt.arbor_grpo.config import ArborTrainConfig
...
class ArborReinforceJob(ReinforceJob):
    ...
    def __init__(self, lm: "LM", train_kwargs: ArborTrainConfig | dict[str, Any]):
        self.lm = lm
        if isinstance(train_kwargs, ArborTrainConfig):
            cfg = train_kwargs
        else:
            # Convert dict to dataclass by filtering to known fields
            default_cfg = ArborTrainConfig()
            allowed_keys = set(default_cfg.to_dict().keys())
            filtered_overrides = {k: v for k, v in (train_kwargs or {}).items() if k in allowed_keys}
            cfg = ArborTrainConfig(**filtered_overrides)

        if cfg.num_generations is None:
            raise ValueError("ArborTrainConfig.num_generations must be set for GRPO training.")
