...
class LMToolCallPart(LMBasePart):
    ...
    Use `dspy.LMToolCall(...)` as a shorter public alias when constructing
...
def Assistant(  # noqa: N802
    *parts: Any, name: str | None = None, metadata: dict[str, Any] | None = None) -> LMMessage:
    ...
    See Also:
        [`dspy.User`][dspy.User]
        [`dspy.LMToolCall`][dspy.LMToolCall]
        [`dspy.ToolResult`][dspy.ToolResult]
    """
    return LMMessage(role="assistant", parts=[_coerce_part(part) for part in parts], name=name, metadata=metadata or {})


LMToolCall = LMToolCallPart
"""Create a tool-call part for an assistant message.

`LMToolCall` is an alias for `LMToolCallPart`. Use it inside `dspy.Assistant(...)`
when you want to include a model-requested tool call in a normalized
conversation.

Note: `LMToolCall` is the single-call message helper. The existing
`dspy.ToolCalls` (plural) is the adapter output type for a collection of tool
calls produced by a signature, and is a distinct concept.
"""
...
def _is_dspy_tool_call(value: Any) -> bool:
    return (
        value.__class__.__name__ in {"LMToolCall", "LMToolCallPart"} and hasattr(value, "name") and hasattr(value, "args")
    )
