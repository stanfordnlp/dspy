...
# Use a deque with a fixed max length so that old entries are evicted automatically
...
class BaseLM:
    ...
    def update_history(self, entry):
        ...
        # Global LM history — deque(maxlen=MAX_HISTORY_SIZE) evicts oldest entry automatically.
        GLOBAL_HISTORY.append(entry)

        if settings.max_history_size == 0:
            return

        # dspy.LM.history — use popleft() (O(1)) instead of pop(0) (O(n))
