...
class StreamingLM(TextOnlyLM):
    def forward_stream(self, request):
        yield dspy.lm.LMStreamEndEvent(response=dspy.LMResponse.from_text("ok", model=request.model))
...
def test_call_normalizes_multimodal_request_before_forward():
    ...
    assert lm.requests[-1].messages[0].parts == [
        dspy.lm.LMTextPart(text="describe"),
        dspy.lm.LMImagePart(data="abc", media_type="image/png"),
    ]
