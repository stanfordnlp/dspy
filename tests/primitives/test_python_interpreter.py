...
def test_get_variable_string():
    """Test retrieving a string variable from sandbox."""
    with PythonInterpreter() as interp:
        interp.execute("my_string = 'hello world'")
        result = interp.get_variable("my_string")
        assert result == "hello world"


def test_get_variable_list():
    """Test retrieving a list variable from sandbox."""
    with PythonInterpreter() as interp:
        interp.execute("my_list = [1, 2, 3, 4, 5]")
        result = interp.get_variable("my_list")
        assert result == [1, 2, 3, 4, 5]


def test_get_variable_dict():
    """Test retrieving a dict variable from sandbox."""
    with PythonInterpreter() as interp:
        interp.execute("my_dict = {'key': 'value', 'count': 42}")
        result = interp.get_variable("my_dict")
        assert result == {"key": "value", "count": 42}


def test_get_variable_nested():
    """Test retrieving a nested structure from sandbox."""
    with PythonInterpreter() as interp:
        interp.execute("nested = {'items': [1, 2, 3], 'meta': {'name': 'test'}}")
        result = interp.get_variable("nested")
        assert result == {"items": [1, 2, 3], "meta": {"name": "test"}}


def test_get_variable_after_computation():
    """Test retrieving a variable created during computation."""
    with PythonInterpreter() as interp:
        interp.execute("result = sum(range(10))")
        result = interp.get_variable("result")
        assert result == 45


def test_get_variable_nonexistent():
    """Test get_variable with non-existent variable raises error."""
    from dspy.primitives.code_interpreter import CodeInterpreterError

    with PythonInterpreter() as interp:
        with pytest.raises(CodeInterpreterError, match="Failed to get variable"):
            interp.get_variable("nonexistent_var")


def test_get_variable_non_json_serializable():
    """Test get_variable with non-JSON-serializable value returns string."""
    with PythonInterpreter() as interp:
        interp.execute("import re; my_regex = re.compile(r'\\d+')")
        result = interp.get_variable("my_regex")
        # Should return string representation
        assert isinstance(result, str)
        assert "re.compile" in result


def test_get_variable_preserves_state():
    """Test that get_variable doesn't affect sandbox state."""
    with PythonInterpreter() as interp:
        interp.execute("x = 10")
