...
def test_adapter_retry_logic(demos):
    adapter = Adapter()
    ...
    with dspy.context(adapter_retry_count=1, lm=lm):
        ...
        with (
            patch.object(adapter, "format", return_value="formatted_input") as mock_format,
            patch.object(
                adapter,
                "parse",
                side_effect=lambda signature, completion: TypeAdapter(signature).validate_python(completion),
            ) as mock_parse,
        ):
