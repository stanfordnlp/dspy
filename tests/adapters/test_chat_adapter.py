...
@pytest.mark.parametrize(
    "adapter",
    [dspy.ChatAdapter(), dspy.JSONAdapter(), dspy.XMLAdapter()],
)
def test_system_message_excludes_natively_formatted_history_field(adapter):
    # The history field is rendered as native multiturn messages, so the system message must not
    # describe a `history` input field the user message will never contain.
    class HistorySignature(dspy.Signature):
        history: dspy.History = dspy.InputField()
        question: str = dspy.InputField()
        answer: str = dspy.OutputField()

    history = dspy.History(messages=[{"question": "What is 1+1?", "answer": "2"}])
    messages = adapter.format(HistorySignature, [], {"history": history, "question": "What is 2+2?"})

    assert messages[0]["role"] == "system"
    assert "`history` (History)" not in messages[0]["content"]
