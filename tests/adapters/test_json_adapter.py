...
def test_json_adapter_passes_structured_output_when_supported_by_model():
    ...
    _, call_kwargs = format_messages_and_lm_kwargs(
        dspy.JSONAdapter(), TestSignature, [], {"input1": "Test input"}, lm=dspy.LM(model="openai/gpt-4o")
    )
...
def test_json_adapter_falls_back_when_structured_outputs_fails():
    ...
    with mock.patch("litellm.completion") as mock_completion:
        mock_completion.side_effect = [
            ...
            ModelResponse(choices=[Choices(message=Message(content="{'output1': 'Test output'}"))], model="openai/gpt-4o"),
        ]
        program(input1="Test input")
...
def test_json_adapter_with_structured_outputs_does_not_mutate_original_signature():
    ...
    original_output_fields = TestSignature.output_fields
    format_messages_and_lm_kwargs(
        dspy.JSONAdapter(), TestSignature, [], {"input1": "Test input"}, lm=dspy.LM(model="openai/gpt-4o")
    )

    assert TestSignature.output_fields == original_output_fields
