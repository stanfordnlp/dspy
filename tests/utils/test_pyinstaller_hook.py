...
def test_hook_is_registered_and_names_existing_files():
    ...
    # The entry point is in the installed distribution's metadata; an older
    # editable install may predate it, in which case only the files are pinned.
    for entry in entry_points(group="pyinstaller40"):
        if entry.value.startswith("dspy."):
            assert entry.load()() == hook_dirs
