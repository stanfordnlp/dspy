...
def test_submit_tool_accepts_structured_output():
    ...
    submit = _build_submit_tool(StructuredSignature)

    assert submit(answer={"text": "done", "score": 0.9}) == {"answer": Answer(text="done", score=0.9)}


def test_forward_with_structured_submit_output():
    class Answer(pydantic.BaseModel):
        text: str
        score: float
        ...
        [
            {
                ...
                "tool_calls": [{"name": "submit", "args": {"answer": {"text": "done", "score": 0.9}}}],
            },
        ]
    )
    dspy.configure(lm=lm)

    result = ReActV2(StructuredSignature, tools=[add])(question="finish")

    assert result.answer == Answer(text="done", score=0.9)
    assert result.history.messages[-1]["answer"] == Answer(text="done", score=0.9)
...
def test_basic_forward_with_submit_records_history_messages():
    ...
    second_call_user_messages = [
        message["content"] for message in lm.history[1]["messages"] if message["role"] == "user"
    ]
...
def test_forward_with_native_tool_calling_renders_tool_results_as_tool_messages():
    ...
    assert any(
        message["role"] == "tool" and message["tool_call_id"] == "call_add" for message in lm.calls[1]["messages"]
    )
    assert lm.calls[0]["kwargs"]["tools"][0]["function"]["name"] == "add"


def test_react_signature_defaults_tool_call_results_to_none():
    react = ReActV2("question -> answer", tools=[add])
    ...
    assert field.annotation == dspy.ToolCallResults
...
def test_forward_accepts_serialized_history():
    lm = DummyLM(
        [
            {"next_thought": "I have the answer.", "tool_calls": [{"name": "submit", "args": {"answer": "3"}}]},
        ]
    ...
    result = react(history={"messages": [{"question": "What is 1+2?"}]})

    assert result.answer == "3"
    assert isinstance(result.history, dspy.History)
    assert result.history.messages[0] == {"question": "What is 1+2?"}
