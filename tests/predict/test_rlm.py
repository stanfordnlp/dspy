...
class TestRLMInitialization:
    ...
    def test_llm_query_sends_images_as_multimodal_content(self):
        ...
        assert lm.call_args.kwargs["messages"][0]["content"] == [
            {"type": "text", "text": "What is this?"},
            {"type": "image_url", "image_url": {"url": image.url}},
        ]

    def test_llm_query_rejects_images_for_text_lm_without_consuming_budget(self):
        from unittest.mock import MagicMock
        ...
        with pytest.raises(ValueError, match="model_type='text' does not support images"):
            tools["llm_query"]("Caption", images="https://example.com/image.png")
        assert tools["llm_query"]("Answer") == "valid text response"
    ...
    def test_llm_query_batched_supports_images_per_prompt(self):
        ...
        tools = RLM("context -> answer", sub_lm=lm)._make_llm_tools()
        first = dspy.Image("data:image/png;base64,Zmlyc3Q=")
        second = dspy.Image("https://example.com/second.png")

        assert tools["llm_query_batched"](["inspect", "text", "compare"], images=[first, None, [first, second]]) == [
            "result",
            "result",
            "result",
        ]
        ...
        for call in lm.call_args_list:
            if call.kwargs:
                ...
                calls[content[0]["text"]] = content[1:]
            else:
                calls[call.args[0]] = []
        assert calls == {"inspect": [first.format()[0]], "text": [], "compare": [first.format()[0], second.format()[0]]}
...
class TestPrepareSerializableVars:
    ...
    def test_preloads_deduplicated_serializable_packages(self):
        ...
        RLM("first, second -> answer")._preload_sandbox_packages(
            {"first": PackageSerializable(), "second": PackageSerializable()}, mock
        )
    ...
    def test_rejects_invalid_sandbox_package_declarations(self, packages):
        ...
        with pytest.raises(TypeError, match="must return a list of non-empty strings"):
            RLM("data -> answer")._preload_sandbox_packages({"data": InvalidPackageSerializable()}, MockInterpreter())
...
def test_image_batch_round_trips_from_sandbox_to_multimodal_llm():
    ...
    result = rlm(image=image, interpreter_factory=PythonInterpreter)
...
def test_image_round_trips_through_typed_rlm_output():
    ...
    rlm.generate_action = make_mock_predictor(
        [
            ...
            {"reasoning": "Return", "code": "SUBMIT(DSPyImage.from_cv2(edited))"},
        ]
    )
    result = rlm(source=source, interpreter_factory=PythonInterpreter)
