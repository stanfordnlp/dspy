...
def test_refine_fail_count_resets_across_calls():
    ...
    predict = DummyModule("question -> answer", always_raise)
    refine = Refine(module=predict, N=5, reward_fn=lambda _, __: 1.0, threshold=1.0, fail_count=2)
...
def test_refine_fail_count_zero_raises_on_first_failure():
    ...
    predict = DummyModule("question -> answer", always_raise)
    refine = Refine(module=predict, N=3, reward_fn=lambda _, __: 1.0, threshold=1.0, fail_count=0)
