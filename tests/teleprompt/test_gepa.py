...
def test_gepa_adapter_disables_logging_on_minibatch_eval(
    ...
    monkeypatch.setattr(
        bootstrap_trace_module, "bootstrap_trace_data", dummy_bootstrap_trace_data
    )
    monkeypatch.setattr(
...
def test_workflow_with_custom_instruction_proposer_and_component_selector():
    ...
    class TimeReader(dspy.Module):
        ...
        def forward(self, clock_photo: dspy.Image):
            ...
            return dspy.Prediction(
                hour=hour_prediction.hour, minute=minute_prediction.minute
            )
    ...
    def metric(example, prediction, trace=None, pred_name=None, pred_trace=None):
        ...
        score = -abs(
            target_hour * 60 + target_minute - (predicted_hour * 60 + predicted_minute)
        )
    ...
    with open(
        "tests/teleprompt/gepa_dummy_lm_custom_component_selector_custom_instruction_proposer.json"
    ) as f:
        data = json.load(f)
...
def component_selection_metric(
    example, prediction, trace=None, pred_name=None, pred_trace=None
):
...
def test_component_selector_functionality():
    ...
    assert (
        "classifier" in selector_calls[0]["components"]
    ), "Should receive all available components"
    assert (
        "generator" in selector_calls[0]["components"]
    ), "Should receive all available components"
...
def test_component_selector_string_all():
    ...
    assert (
        candidates_round_robin[1].classifier.signature.instructions
        == original_classifier_instruction
        and candidates_round_robin[1].generator.signature.instructions
        != original_generator_instruction
    ) or (
        candidates_round_robin[1].classifier.signature.instructions
        != original_classifier_instruction
        and candidates_round_robin[1].generator.signature.instructions
    ...
    assert (
        candidates_all[1].classifier.signature.instructions
        != original_classifier_instruction
        and candidates_all[1].generator.signature.instructions
        != original_generator_instruction
