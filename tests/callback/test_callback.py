...
def test_interpreter_callbacks():
    ...
    class Callback(BaseCallback):
        def _record(self, handler, call_id, **kwargs):
            calls.append({"handler": handler, "call_id": call_id, "parent": ACTIVE_CALL_ID.get(), **kwargs})

        def on_interpreter_execute_start(self, call_id, instance, inputs):
            self._record("execute_start", call_id, inputs=inputs)

        def on_interpreter_execute_end(self, call_id, outputs, exception):
            self._record("execute_end", call_id, outputs=outputs, exception=exception)

        def on_interpreter_tool_call_start(self, call_id, instance, inputs):
            self._record("tool_start", call_id, inputs=inputs)

        def on_interpreter_tool_call_end(self, call_id, outputs, exception):
            self._record("tool_end", call_id, outputs=outputs, exception=exception)

        def on_interpreter_startup_start(self, call_id, instance, inputs):
            self._record("startup_start", call_id, inputs=inputs)

        def on_interpreter_startup_end(self, call_id, outputs, exception):
            self._record("startup_end", call_id, outputs=outputs, exception=exception)

        def on_interpreter_shutdown_start(self, call_id, instance, inputs):
            self._record("shutdown_start", call_id, inputs=inputs)

        def on_interpreter_shutdown_end(self, call_id, outputs, exception):
            self._record("shutdown_end", call_id, outputs=outputs, exception=exception)

    class Interpreter:
        def __init__(self):
            self.tools = {"echo": lambda value: value}
        ...
        def execute(self, code, variables=None):
            return self.invoke_tool("echo", {"value": code})
    ...
    assert [call["handler"] for call in calls] == [
        "startup_start", "startup_end", "execute_start", "tool_start",
        "tool_end", "execute_end", "shutdown_start", "shutdown_end",
    ]
    assert calls[2]["inputs"] == {"code": "result", "variables": {"x": 1}}
    assert calls[3]["parent"] == calls[2]["call_id"]
