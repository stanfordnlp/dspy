...
class T5EncoderRerank(torch.nn.Module):
    def __init__(self, model_type_or_dir):
        super().__init__()
        self.model = T5EncoderModel.from_pretrained(model_type_or_dir, torch_dtype="auto")
...
class T5EncoderRerankWrapper:
    def __init__(self, model_type_or_dir="t5-3b", path_checkpoint="trecdl22-crossencoder-rankT53b-repro/pytorch_model.bin"):
        super().__init__()
        self.model = T5EncoderRerank(model_type_or_dir)
        self.model.load_state_dict(torch.load(path_checkpoint, map_location=torch.device("cpu")))
    ...
    def __call__(self, query, passages, batch_size: int = 4):
        ...
        for start_idx in range(0, len(passages), batch_size):
            ...
            features = self.tokenizer(to_tokenize, padding=True, truncation=True, return_tensors="pt").to(self.device)
            with torch.no_grad():
...
def retrieveRerankEnsembleAvg(
    ...
    for q in queries:
        ...
        for idx in passages_cs_scores_sorted[::-1]:
            ...
            passages_org[psg.long_text] = (
                passages_org.get(psg.long_text, 0) + psg.prob
            )
    ...
    passages_org_scores = sorted([(score, text) for text, score in passages_org.items()], reverse=True)
